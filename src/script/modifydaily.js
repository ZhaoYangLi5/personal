...
require(['jquery', 'jquery-weui', 'datepicker', 'template', 'cookie'], function() {
    ...
    function addOption(jobCls) {
        $.each(jobCls, function(i, item) {
            var opt = $("<option></option>").html(item).val(item);
    ...
    function getJson(data) {
        ...
        $("#title").html(name);
        $("#jobType").find("select").val(data.DAYREPORT_TYPE);
        var date = new Date(data.TASKEXEM_SUBMITDATE).pattern("yyyy-MM-dd EE")
        $(".ui-datepicker-curr-month").text(date);
        $("#project").val(data.PROJECT_NAME_FULL);
        $("#jobContent").find("textarea").html(data.TASKEXEM_CONTENT);
        if (data.TASKM_CG) {
            $("#jobResults").find("textarea").html(data.TASKM_CG);
        }
    }
